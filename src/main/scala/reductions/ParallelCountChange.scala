...
object ParallelCountChange {
  ...
  def countChange(money: Int, coins: List[Int]): Int = {
    def countChangeAcc(money: Int, coins: List[Int], acc: Int): Int =
      if (money < 0) acc
      else if (money == 0) acc + 1 else if (coins.isEmpty) acc
      else if (coins.head > money) countChangeAcc(money, coins.tail, acc)
      else countChangeAcc(money - coins.head, coins, countChangeAcc(money, coins.tail, acc))
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if(money == 0) 1
      ...
      val (w, wout) = parallel(parCountChange(money - coins.head, coins, threshold),
