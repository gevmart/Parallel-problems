...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    while (i < input.length) {
      angle = input(i) / i
      output(i) = if (angle > output(i - 1)) angle else output(i - 1)
      i += 1
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    var max = startingAngle //if (from != 0) output(from - 1) else 0f
    var i = from
    var angle = 0f
    while (i < until) {
      angle = input(i) / i
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
                tree: Tree): Unit = {
    tree match {
      case Leaf(from, until, angle) => downsweepSequential(input, output, startingAngle, from, until)
